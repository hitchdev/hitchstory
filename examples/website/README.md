...
This example project demonstrates a **best practices** and **state of the art features** for **hermetic end to end web app testing**:

It is set up to test a Django "to do" project (in the app folder) but it is designed to such that you can swap out the django app with another containerized app easily. The code in the tests/ folder can remain the same.
...
* Absolute and total environmental consistency and portability (Mac/WSL/Linux) via podman-in-podman containerization and dependency pinning.
...
* Database fixtures on Postgres which can be trivially configured in-story (currently tied to Django, but the code can be changed to use a different database quite easily).
* Story inheritance with fixture merging - the pre-existing todos in the "Login" story are combined with the todos in the "add and retrieve todo" story that inherits from it.
...
* Simple 3 step github actions config to run all of the tests (& upload any artefacts).
* Explicitly waiting for ports to open before kicking off the test (eliminates a common source of flakiness).
* coverage.py is preconfigured to spit out XML and HTML reports.

State of the art features enabled by hitchstory:

* Type-safe high level specifications in StrictYAML to maintain a separation of concerns between specification and test execution code - [all the simplicity and readability of YAML, with none of the headaches](https://hitchdev.com/hitchstory/why/strictyaml).
* Autogeneration of beautiful markdown documentation based upon storytests - complete with screenshots and playable GIFs of the story.
* Storytests which can be programmed to rewrite themselves from program output (see example below).

It is a work in progress. These are more things I'll be adding soon:

- [ ] Snapshot testing of emails sent via SMTP.
