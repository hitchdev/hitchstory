...
class DocGivenProperty(object):
    ...
    def documentation(self):
        return Template(self._docstory.templates["given"][self._name]).render(**{self._name: self._given_property})
...
class DocGivenProperties(object):
    ...
    def items(self):
        return  [(name, DocGivenProperty(self._docstory, name, given_property)) for name, given_property in self._docstory.story.given.items()]
...
class DocStory(object):
    def __init__(self, story):
        ...
        self._slugified_templates = {
            ...
            "steps": {to_underscore_style(name): text for name, text in self.templates["steps"].items()},
            "given": {slugify(name): text for name, text in self.templates["given"].items()},
        }
